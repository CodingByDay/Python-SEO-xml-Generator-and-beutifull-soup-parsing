...
namespace Emmares4.Extensions
{
    public class DataTableParser<T> where T : class
    {
        ...
        public IDictionary<int, PropertyMap> Properties { get => _propertyMap; set => _propertyMap = value; }

        public DataTableParser(IEnumerable<KeyValuePair<string, StringValues>> configParams, IQueryable<T> queryable)
        {
        ...
        public Results<T> Parse()
        {
            ...
            var hasFilterText = !string.IsNullOrWhiteSpace(_config[Constants.SEARCH_KEY]) || Properties.Any(p => !string.IsNullOrWhiteSpace(p.Value.Filter));
        ...
        public DataTableParser<T> SetConverter(Expression<Func<T, object>> property, Expression<Func<T, string>> tostring)
        {
        ...
        private void ApplySort()
        {
            ...
            foreach (var param in _config.Where(k => Regex.IsMatch(k.Key, Constants.ORDER_PATTERN)))
            {
                ...
                if (!Properties.ContainsKey(sortcolumn) || !Properties[sortcolumn].Orderable)
                {
                    continue;
                }
                ...
                var sortProperty = Properties[sortcolumn].Property;
            ...
            if (!sorted)
            {
                var firstProp = Expression.Property(paramExpr, Properties.First().Value.Property);
                var propType = Properties.First().Value.Property.PropertyType;
                var delegateType = Expression.GetFuncType(_type, propType);
        ...
        private bool EnumerablFilter(T item)
        {
            ...
            foreach (var map in Properties.Where(m => m.Value.Searchable))
            {
        ...
        private Expression<Func<T, bool>> GenerateEntityFilter()
        {
            ...
            foreach (var propMap in Properties.Where(m => m.Value.Searchable))
            {
